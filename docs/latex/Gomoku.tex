...
A principal motivação do projeto foi aplicar os conceitos e técnicas abordados na disciplina em um cenário prático. A escolha de um jogo \textit{multiplayer} estratégico permite grande liberadade na escolha e aplicação das tecnologias de \textit{backend} e \textit{frontend} a serem utilizadas. O desafio consistia em construir um sistema robusto, e que integrasse as diversas ferramentas exigidas pelo plano de ensino, como o \textit{videochat} e o protocolo HTTPS.

\subsection{Problema}
O problema central do projeto foi desenvolver uma aplicação \textit{full-stack} que suportasse múltiplos modos de jogo (PvP online, PvP local e PvE contra IA), autenticação de usuários, comunicação em tempo real e funcionalidades de mídia, como \textit{videochat} P2P e gravação de partidas. A solução deveria seguir o padrão de projeto MVC (Model-View-Controller), ser responsiva (desktop e mobile) e atender a rigorosos requisitos de segurança e infraestrutura, incluindo o \textit{deploy} obrigatório em um servidor VPS-UFSC.

%\subsection{Trabalhos Relacionados}
\subsection{Trabalhos Relacionados}
Existem diversas implementações do jogo Gomoku disponíveis, tanto em plataformas web quanto mobile. Algumas das mais notáveis incluem:

\begin{itemize}
    \item \textbf{Gomoku Online (gomoku.com)}: Uma plataforma popular que oferece partidas contra IA e outros jogadores. Utiliza um sistema de ranking ELO similar ao proposto neste trabalho, mas carece de recursos de vídeo em tempo real.
    \item \textbf{Renju Portal}: Focado na variante profissional do Gomoku (Renju), possui regras mais estritas e uma comunidade competitiva forte. A interface é funcional, porém datada.
    \item \textbf{Implementações Open Source no GitHub}: Diversos projetos utilizam tecnologias como Socket.IO e Node.js para prover jogabilidade em tempo real. No entanto, poucos integram recursos avançados como gravação de partidas e videochat diretamente na aplicação.
\end{itemize}

O presente projeto se diferencia ao integrar, em uma única plataforma, o jogo clássico, sistema de ranking, comunicação por vídeo (WebRTC) e gravação de partidas (FFMPEG), atendendo aos requisitos da disciplina de Programação Web.
...
\begin{itemize}
    ...
    \item \textbf{HTTPS:} O \textit{deploy} no VPS utiliza Nginx como \textit{proxy} reverso com certificados SSL/TLS (Let's Encrypt), garantindo a criptografia.
...
\section{Conclusão}
\label{sec:conclusao}
O desenvolvimento do projeto Gomoku Web atingiu seus objetivos principais, entregando uma aplicação \textit{full-stack} robusta que cumpre os requisitos básicos do jogo, mas também implementa um conjunto de funcionalidades avançadas e complexas.
...
Os desafios encontrados, especialmente na estabilidade da comunicação em tempo real e no \textit{design} responsivo, foram superados e resultaram em um sistema mais resiliente e polido.

\begin{thebibliography}{10} 

% requisitos do porjeto
