...
\lstdefinestyle{mystyle}{
    ...
    commentstyle=\color{codegreen},      % Verde restaurado
    keywordstyle=\color{magenta},        % Magenta restaurado
    numberstyle=\tiny\color{codegray},
    stringstyle=\color{codepurple},      % Roxo restaurado
...
\hypersetup{
  ...
  linkcolor=black,    % Links internos (Figuras/Seções) em PRETO (remove vermelho)
  citecolor=black,    % Citações [1] em PRETO (remove verde)
  filecolor=black,
  pdfborder={0 0 0},
}
...
\subsection{Problema}
O problema central do projeto foi desenvolver uma aplicação \textit{full-stack} que suportasse múltiplos modos de jogo (PvP online, PvP local e PvE contra IA), autenticação de usuários, comunicação em tempo real e funcionalidades de mídia, como a gravação de partidas. A solução deveria seguir o padrão de projeto MVC (Model-View-Controller), ser responsiva (desktop e mobile) e atender a rigorosos requisitos de segurança e infraestrutura, incluindo o \textit{deploy} obrigatório em um servidor VPS-UFSC.
...
\caption{Interface da partida, exibindo o tabuleiro, o chat em tempo real}
\label{fig:tela-jogo}
\end{figure}
