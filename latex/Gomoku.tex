...
A implementação computacional do Gomoku tem sido objeto de estudo tanto na engenharia de software quanto na inteligência artificial. Uma abordagem seminal foi apresentada por Freisleben \cite{freisleben:nn}, que desenvolveu uma rede neural capaz de aprender a jogar Gomoku através de treinamento supervisionado, demonstrando que arquiteturas conexionistas poderiam capturar as nuances estratégicas do jogo sem depender exclusivamente de algoritmos de busca exaustiva.

Enquanto o trabalho de Freisleben foca na otimização do agente inteligente e na predição de movimentos baseada em padrões aprendidos, este projeto adota uma abordagem de sistemas distribuídos. O foco aqui não é apenas a eficiência algorítmica da IA, mas a construção de uma arquitetura \textit{web} robusta que integre persistência de dados, comunicação em tempo real via WebSockets e processamento de mídia.
...
Além dos \textit{frameworks} principais, o projeto utiliza um ecossistema de bibliotecas específico para implementação de todos os requisitos sem conflitos.

\subsubsection{Backend (Python)}
As dependências gerenciadas pelo \texttt{pip} e listadas no \texttt{requirements.txt} incluem:
\begin{itemize}
    \item \textbf{Motor:} Driver assíncrono para MongoDB, essencial para manter a natureza não-bloqueante do FastAPI durante operações de banco de dados.
    \item \textbf{Python-jose:} Biblioteca utilizada para a criação, assinatura e verificação de tokens JWT (JSON Web Tokens) no processo de autenticação.
    \item \textbf{Passlib [bcrypt]:} Responsável pelo \textit{hashing} seguro e verificação de senhas de usuários, garantindo que credenciais não sejam armazenadas em texto plano.
    \item \textbf{FFmpeg-python:} \textit{Wrapper} que permite a orquestração de comandos do FFMPEG diretamente via código Python para o processamento de vídeos das partidas.
    \item \textbf{Pydantic:} Fundamental para a validação de dados, serialização e definição de esquemas de entrada e saída da API.
\end{itemize}

\subsubsection{Frontend (Node.js)}
No ecossistema JavaScript/TypeScript, as principais bibliotecas incluem:
\begin{itemize}
    \item \textbf{React Router Dom:} Gerencia a navegação e o roteamento do lado do cliente (\textit{Client-Side Routing}), permitindo a construção de uma \textit{Single Page Application} (SPA) fluida.
    \item \textbf{Axios:} Cliente HTTP baseado em \textit{promises}, utilizado para realizar todas as requisições assíncronas para a API REST do \textit{backend}.
    \item \textbf{TypeScript:} Adiciona tipagem estática ao projeto, aumentando a segurança do código e facilitando a manutenção e refatoração.
\end{itemize}
...
\caption{Interface do lobby de jogos, mostrando a seleção de modo de jogo e a lista de jogadores no rank.}
\label{fig:tela-lobby}
\end{figure}

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.8\columnwidth]{figs/pvplobby.png}
\caption{Interface de partida PvP online.}
\label{fig:tela-jogo}
\end{figure}
...
Para validar a robustez das implementações de segurança, a aplicação foi submetida a testes de intrusão controlados utilizando ferramentas padrão da indústria.

O \textit{Burp Suite} foi utilizado como um \textit{proxy} de interceptação para analisar o tráfego HTTP e WebSocket entre o cliente React e o servidor FastAPI. Foram realizados testes de manipulação de dados em tempo real (\textit{Man-in-the-Middle}), onde \textit{payloads} JSON enviados via WebSocket foram interceptados e modificados na tentativa de executar movimentos ilegais no tabuleiro ou personificar outros jogadores. O sistema demonstrou resiliência, com o \textit{backend} validando corretamente a integridade das mensagens e rejeitando dados que não correspondiam ao estado atual do jogo ou à assinatura do \textit{token} JWT.

Adicionalmente, utilizou-se a ferramenta \textit{Surfshark VPN} para simular conexões a partir de múltiplos endereços IP e localizações geográficas distintas. O objetivo foi verificar a estabilidade da conexão persistente (WebSocket) sob latências variadas e testar se o servidor expunha informações sensíveis da infraestrutura interna (como IPs reais da rede Docker) durante trocas de rota. Os testes confirmaram que o Nginx atuou como \textit{gateway}, anonimizando a topologia da rede interna.

\subsection{Soluções de Segurança}
...
A estrutura final de arquivos do projeto, após a exclusão de dependências e arquivos de configuração de ambiente local, organiza-se da seguinte maneira, evidenciando a separação clara entre \textit{frontend}, \textit{backend} e infraestrutura:
...
\bibitem{freisleben:nn}
B. Freisleben, "A neural network that learns to play five-in-a-row," in \emph{Proceedings of 1994 IEEE International Conference on Neural Networks (ICNN'94)}, vol. 7, pp. 4602-4606, 1994.
