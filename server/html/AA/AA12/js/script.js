...
// No-ops usados apenas como marcadores para seleção por atributo no HTML
window.statusNome = function() { /* placeholder */ };
window.statusSobrenome = function() { /* placeholder */ };
...
function setupNovaContaValidation() {
    ...
    createButton.addEventListener('click', () => {
        if (!createButton.disabled) {
            try {
                ...
                // Redireciona para a tela de login e preenche os campos para facilitar o login
                mostrarApenasLogin();
                const loginEmail = document.querySelector('#login-body input[type="text"]');
                const loginPass = document.getElementById('login-password');
                if (loginEmail && loginPass) {
                    loginEmail.value = newAccount.email;
                    loginPass.value = newAccount.senha;
    ...
    // Handler do botão de login: valida e exibe a seção pós-login
    document.addEventListener('DOMContentLoaded', () => {
        const botao = document.getElementById('botaoLogin');
        if (botao) {
            botao.addEventListener('click', () => {
                const emailField = document.querySelector('#login-body input[type="text"]');
                const passwordField = document.getElementById('login-password');
                if (!emailField || !passwordField) return;
                if (!emailField.value.includes('@') || emailField.value.split('@').length - 1 !== 1) {
                    alert('E-mail inválido. Deve conter exatamente um @.');
                    return;
                }
    ...
    // Mostra a área pós-login e carrega os seletores do IBGE
    window.mostrarAposLogin = function() {
        ocultarTudo();
        const post = document.getElementById('post-login');
        if (post) post.style.display = 'block';
        carregarUFsEnd();
    }

    async function carregarUFsEnd() {
        const ufEnd = document.getElementById('uf_end');
        const municipioEnd = document.getElementById('municipio_end');
        if (!ufEnd || !municipioEnd) return;
        try {
            ufEnd.innerHTML = '<option value="">Carregando...</option>';
            const response = await fetch('https://servicodados.ibge.gov.br/api/v1/localidades/estados');
            const estados = await response.json();
            estados.sort((a, b) => a.nome.localeCompare(b.nome));
            ufEnd.innerHTML = '<option value="">Selecione o estado</option>';
            estados.forEach(estado => {
                ufEnd.innerHTML += `<option value="${estado.sigla}">${estado.nome}</option>`;
            });
            ufEnd.disabled = false;
            ufEnd.addEventListener('change', (e) => {
                municipioEnd.innerHTML = '<option value="">Selecione o município</option>';
                if (e.target.value) carregarMunicipiosEnd(e.target.value);
                else municipioEnd.disabled = true;
            });
        } catch (err) {
            console.error('Erro ao carregar UFs (end):', err);
            ufEnd.innerHTML = '<option value="">Erro ao carregar</option>';
        }
    }

    async function carregarMunicipiosEnd(uf) {
        const municipioEnd = document.getElementById('municipio_end');
        if (!municipioEnd) return;
        try {
            municipioEnd.disabled = true;
            municipioEnd.innerHTML = '<option value="">Carregando...</option>';
            const response = await fetch(`https://servicodados.ibge.gov.br/api/v1/localidades/estados/${uf}/municipios`);
            const municipios = await response.json();
            municipios.sort((a, b) => a.nome.localeCompare(b.nome));
            municipioEnd.innerHTML = '<option value="">Selecione o município</option>';
            municipios.forEach(m => municipioEnd.innerHTML += `<option value="${m.nome}">${m.nome}</option>`);
            municipioEnd.disabled = false;
        } catch (err) {
            console.error('Erro ao carregar municípios (end):', err);
            municipioEnd.innerHTML = '<option value="">Erro ao carregar</option>';
        }
    }
}