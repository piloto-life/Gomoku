...
Cliente TCP simples para testar o servidor da atividade AA11.

Uso:
    python3 tcp_client.py <server_url> [mensagem]

Exemplos:
    python3 tcp_client.py http://pw.alec.coelho.vms.ufsc.br/AA/AA11 "Olá TCP"
    python3 tcp_client.py ws://meu-servidor:8082
...
def interactive_mode(host: str, port: int):
    print("Modo interativo. Digite mensagens (ou 'exit' para sair):")
    try:
        while True:
            msg = input('> ')
            if not msg:
